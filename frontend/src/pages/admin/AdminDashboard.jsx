        ...
        {tab === 'dashboard' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                { icon: '👦', label: 'ENFANTS', value: nbActifs, sub: 'actifs', extra: `+${benefs.length} total`, color: 'from-blue-500 to-arina-blue', bg: 'bg-blue-50', link: 'enfants' },
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  { label: '👦 Nouvel enfant', action: () => { setTab('enfants'); setTimeout(() => openBenefForm(null), 100); }, color: 'bg-blue-100 text-blue-700 hover:bg-blue-200' },
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
              ...
              <div className="space-y-2">
                {[
                  ...
                  { time: '12:15', icon: '👦', text: 'Enfant ajouté : Thomas M. (AR-001)', author: 'Marie', color: 'bg-blue-100 text-blue-700' },
          ...
          <div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              <table className="w-full text-sm"><thead className="bg-gray-50 text-left"><tr><th className="px-4 py-3 font-semibold">Titre</th><th className="px-4 py-3 font-semibold">Catégorie</th><th className="px-4 py-3 font-semibold">Date</th><th className="px-4 py-3 font-semibold">Vues</th><th className="px-4 py-3 font-semibold">Actions</th></tr></thead>
                <tbody className="divide-y divide-gray-100">{news.map((n) => (<tr key={n.id} className="hover:bg-gray-50"><td className="px-4 py-3 font-medium text-arina-dark max-w-xs truncate">{n.title}</td><td className="px-4 py-3"><span className="px-2 py-0.5 bg-gray-100 rounded-full text-xs">{n.category}</span></td><td className="px-4 py-3 text-arina-gray text-xs">{n.date}</td><td className="px-4 py-3">👁️ {n.views || 0}</td><td className="px-4 py-3"><div className="flex gap-1"><Link to={`/actualites/${n.slug || n.id}`} target="_blank" rel="noopener noreferrer" className="px-2 py-1 text-xs bg-gray-50 rounded hover:bg-gray-100">👁️</Link><button onClick={() => openNewsForm(n)} className="px-2 py-1 text-xs bg-blue-50 text-blue-600 rounded hover:bg-blue-100">✏️</button><button onClick={() => removeNews(n.id)} className="px-2 py-1 text-xs bg-red-50 text-red-600 rounded hover:bg-red-100">🗑️</button></div></td></tr>))}</tbody></table>
            </div>
          </div>
        )}
